...
async function refreshData() {
    ...
    state.items = await window.electronAPI.getItems();
    state.invoices = await window.electronAPI.getInvoices();
}
...
window.handleSaveCategory = async () => {
    ...
    if (id) {
        await window.electronAPI.updateCategory(data);
    } else {
...
window.generateInvoice = async () => {
    ...
    const invId = await window.electronAPI.addInvoice({ invoice, items: state.cart });
    ...
    await window.electronAPI.printToPDF(html);
    ...
    if (choice) {
        ...
        window.electronAPI.openExternal(`https://wa.me/${phone}?text=${shareText}`);
    }
